...
/*#__PURE__*/
function () {
  var _ref = (0, _asyncToGenerator2.default)(function* (pagePath, directory) {
    ...
    try {
      const pageData = yield pageDataUtil.read({
        publicDir
      }, pagePath);
      return {
  ...
  return function getCachedPageData(_x, _x2) {
...
class WebsocketManager {
  ...
  init({
    ...
    this.websocket.on(`connection`, s => {
      ...
      /*#__PURE__*/
      function () {
        var _ref2 = (0, _asyncToGenerator2.default)(function* (path) {
          if (!_this.pageResults.has(path)) {
            const result = yield getCachedPageData(path, _this.programDir);

            if (result) {
              _this.pageResults.set(path, result);
            } else {
          ...
          _this.websocket.send({
            ...
            why: `getDataForPath`,
            payload: _this.pageResults.get(path)
          });

          telemetry.trackCli(`WEBSOCKET_PAGE_DATA_UPDATE`, {
            siteMeasurements: {
              clientsCount: _this.connectedClients,
              paths: hashPaths(Array.from(_this.activePaths))
            }
          }, {
            debounce: true
          });
        ...
        return function getDataForPath(_x3) {
