...
const _require5 = require(`./trace-svg`),
      ...
      notMemoizedtraceSVG = _require5.notMemoizedtraceSVG;
...
const cachifiedProcess =
/*#__PURE__*/
function () {
  var _ref = (0, _asyncToGenerator2.default)(function* (_ref2, genKey, processFn) {
    let cache = _ref2.cache,
        arg = (0, _objectWithoutPropertiesLoose2.default)(_ref2, ["cache"]);
    const cachedKey = genKey(arg);
    const cached = yield cache.get(cachedKey);

    if (cached) {
      return cached;
    }

    const result = yield processFn(arg);
    yield cache.set(cachedKey, result);
    return result;
  });

  return function cachifiedProcess(_x2, _x3, _x4) {
...
function _base() {
  _base = (0, _asyncToGenerator2.default)(function* (arg) {
    if (arg.cache) {
      ...
      return yield cachifiedProcess(arg, generateCacheKey, generateBase64);
    }
...
function _traceSVG() {
  _traceSVG = (0, _asyncToGenerator2.default)(function* (args) {
    if (args.cache) {
      // Not all tranformer plugins are going to provide cache
      return yield cachifiedProcess(args, generateCacheKey, notMemoizedtraceSVG);
    }
...
function getTracedSVG(_x7) {
  return _getTracedSVG.apply(this, arguments);
}

function _getTracedSVG() {
  _getTracedSVG = (0, _asyncToGenerator2.default)(function* ({
    file,
    options,
    cache,
    reporter
...
function _fluid() {
  _fluid = (0, _asyncToGenerator2.default)(function* ({
    ...
    const tracedSVG = yield getTracedSVG({
      options,
      file,
      cache,
      reporter
